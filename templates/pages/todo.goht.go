...
func TodoPage(todo domain.Todo) goht.Template {
	return goht.TemplateFunc(func(ctx context.Context, __w io.Writer) (__err error) {
		...
		__var1 := goht.TemplateFunc(func(ctx context.Context, __w io.Writer) (__err error) {
			...
			if __err = partials.EditTodoForm(todo).Render(ctx, __buf); __err != nil {
				return
			}
			if !__isBuf {
		...
		if __err = shared.Page("Todo").Render(goht.PushChildren(ctx, __var1), __buf); __err != nil {
			return
		}
		if !__isBuf {
